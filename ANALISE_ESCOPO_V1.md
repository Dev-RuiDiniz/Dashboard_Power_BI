...
O projeto evoluiu de forma relevante: o workspace agora esta operacional, os modulos principais de plataforma foram integrados ao runtime, a tela de perfil passou a ter backend real e o frontend deixou de depender de leitura direta do Supabase para dashboard, notifications, exports e settings. Mesmo assim, o repositorio **ainda nao entrega o V1 completo do PDF**. A plataforma hoje tem uma base funcional centralizada na API, mas continua com lacunas importantes em BI avancado, persistencia final de relatorios, export pipeline aderente ao escopo, 2FA e varios pontos do stack descrito no `AGENTS.md`.
...
| T09  | Exportacao PDF/Excel com confirmacao e historico                 | Parcial  | `apps/web/src/app/app/exports/page.tsx`, `apps/web/src/components/exports/exports-list.tsx`, `apps/api/src/platform/exports/*`                                                                                                        | Historico e endpoints existem via API, mas o pipeline final aderente ao PDF ainda e parcial.          |
...
| T14  | Gestao de permissoes                                             | Feito    | `apps/web/src/app/app/admin/permissions/page.tsx`, `apps/web/src/components/admin/admin-permissions.tsx`, `apps/api/src/permissions/*`, `apps/api/src/app.module.ts`                                                                  | O modulo agora participa do runtime principal. A profundidade funcional ainda e menor que a do PDF.   |
| T15  | Gestao de relatorios                                             | Parcial  | `apps/web/src/app/app/admin/reports/page.tsx`, `apps/web/src/components/admin/admin-reports.tsx`, `apps/api/src/reports/report-definitions.admin.controller.ts`, `apps/api/src/reports/repositories/report-definitions.repository.ts` | Fluxo administrativo existe, mas a persistencia segue em memoria.                                     |
...
| T17  | Auditoria com filtros                                            | Feito    | `apps/web/src/app/app/admin/audit/page.tsx`, `apps/web/src/components/admin/admin-audit.tsx`, `apps/api/src/audit/*`, `apps/api/src/app.module.ts`                                                                                    | O modulo esta ligado ao runtime e a tela existe. Continua longe do hardening final do PDF.            |
| T18  | Configuracoes do sistema                                         | Feito    | `apps/web/src/app/app/admin/settings/page.tsx`, `apps/web/src/components/admin/admin-settings.tsx`, `apps/api/src/platform/settings/*`                                                                                                | A tela e a API estao ligadas no runtime atual.                                                        |
...
| RELAT.   | Parcial | Catalogo, visualizacao, filtros e gestao admin basica                             | Definicoes seguem em memoria; exportacao completa do PDF ainda nao fechou                                       | Persistencia final das definicoes, favoritos e export aderente                  |
...
- `PERMISS.`: roles, setores, grupos e a tela de permissoes agora estao no runtime; a implementacao continua parcial frente ao nivel esperado no PDF.
- `SQL`: a camada parametrizada continua sendo um dos pontos mais solidos do repositorio.
- `RELAT.`: catalogo e query estao bons, mas `ReportDefinitionsRepository` continua em memoria.
- `BI`: os KPIs do dashboard home foram centralizados na API, mas o BI descrito no PDF ainda nao foi entregue.
- `ADMIN`: ha mais cobertura real no runtime, mas ainda sem dashboard admin aderente e sem fechamento completo de governanca.
...
- `apps/api/src/reports/repositories/report-definitions.repository.ts` continua em memoria;
- `apps/api/src/platform/exports/*` existe e esta ligado, mas a estrategia final de export pesada ainda nao fecha o PDF;
...
- **Debitos de escopo**: continuam relevantes, principalmente em BI avancado, exportacao aderente ao PDF, persistencia real de relatorios e seguranca do MVP completo.
...
- tirar `ReportDefinitionsRepository` de memoria;
- consolidar exports para um pipeline aderente ao PDF;
...
1. Resolver persistencia real de relatorios e configuracoes de platform.
2. Fechar exportacoes aderentes ao PDF.
...
- [ ] Definicoes de relatorios deixaram de depender de memoria
- [ ] Export pipeline final aderente ao PDF foi fechado
...
- [ ] Relatorios administrativos tem persistencia real
- [ ] Dashboards e favoritos tem persistencia real
- [ ] Exports e auditoria atendem o nivel operacional esperado no PDF
