...
| T07  | Dashboard interativo com graficos, drill-down e interacao rica   | Parcial  | `apps/web/package.json`, `apps/web/src/components/dashboard/dashboard-home.tsx`, `apps/api/src/platform/dashboard/*`, `apps/web/src/lib/platform-api.ts`                                                                                                                                                                                                                                          | A home agora tem charts reais, payload consolidado de BI e um drill-down inicial por KPI, mas ainda faltam exploracao rica, multiplas dimensoes e interacao aderente ao PDF. |
| T08  | Dashboards personalizados, favoritos e widgets                   | Parcial  | `apps/api/src/platform/dashboards/*`, `apps/api/src/reports/report-favorites.service.ts`, `apps/web/src/app/app/dashboards/page.tsx`, `apps/web/src/components/dashboard/dashboard-workspace.tsx`, `apps/web/src/components/reports/report-detail.tsx`, `supabase/migrations/20260604203252_002_create_reports_dashboards.sql`, `supabase/migrations/20260607183000_006_api_favorite_reports.sql` | Ja existe fluxo funcional inicial de dashboards personalizados e favoritos, mas ainda faltam widgets editaveis, dashboard padrao mais completo e o editor visual do PDF.     |
...
| -------- | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| AUTH     | Parcial | Login, refresh, logout, forgot/reset password, rate limit, perfil do usuario, 2FA/TOTP implementado e sessao web em `sessionStorage` com refresh automatico unico | O hardening avancou, mas CSRF, blacklist de tokens e 2FA obrigatorio para admins ainda seguem incompletos | 2FA obrigatorio para admins, endurecimento adicional da sessao e hardening final do PDF |
...
| RELAT.   | Parcial | Catalogo, visualizacao, filtros, gestao admin basica, persistencia real das definicoes, favoritos e pipeline de exportacao com BullMQ + Redis                     | Cobertura mais ampla do PDF ainda nao fechou (storage S3 pendente)                                        | Ampliar favoritos, storage S3 e demais fluxos do escopo                                 |
| BI       | Parcial | Dashboard home com KPIs, resumo por setor, payload consolidado, charts reais via API, drill-down inicial e dashboards personalizados                              | Ainda faltam drill-down rico e editor visual                                                              | Drill-down multi-dimensao, personalizacao mais profunda e editor visual                 |
...
- `AUTH`: login, refresh, reset, rate limit e bcrypt existem; `GET /auth/me` e `PATCH /auth/me/password` agora existem; 2FA/TOTP implementado com setup, verify, disable e login TOTP; a sessao web passou a usar `sessionStorage`, com migracao de legado e refresh automatico unico em `401`.
...
- `RELAT.`: catalogo, query, definicoes administrativas, favoritos e exportacao de relatorios agora usam backend real; o pipeline de exports ja possui BullMQ + Redis com fallback em memoria; o modulo continua parcial pela aderencia mais ampla do PDF (storage S3 pendente).
- `BI`: os KPIs do dashboard home evoluiram para payload consolidado, charts reais, drill-down inicial e dashboards personalizados, mas o BI descrito no PDF ainda nao foi entregue por falta de exploracao mais rica e editor visual.
...
- o BI agora ja possui charts reais na home, mas ainda abaixo do que o PDF espera;
...
- [ ] T07 entregue com BI interativo real e drill-down aderente ao PDF
- [ ] T08 entregue com dashboards personalizados completos e widgets aderentes ao PDF
...
- [x] Dashboards e favoritos tem persistencia real inicial
